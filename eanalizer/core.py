...
def filter_data_by_date(data: List[EnergyData], start_date_str: Optional[str], end_date_str: Optional[str]) -> List[EnergyData]:
    ...
    filtered_list = [d for d in data if (not start_date or d.timestamp >= start_date) and (not end_date or d.timestamp <= end_date)]
...
def aggregate_daily_data(data: List[EnergyData]) -> pd.DataFrame:
    ...
    daily_df = df.groupby('date').agg(pobor_przed=('pobor_przed', 'sum'), oddanie_przed=('oddanie_przed', 'sum'), pobor=('pobor', 'sum'), oddanie=('oddanie', 'sum')).reset_index()
    return daily_df
...
def calculate_optimal_capacity(hourly_data: List[EnergyData], daily_data: pd.DataFrame, tariff_manager: TariffManager, tariff: str):
    ...
    if not net_import_days.empty:
        hourly_df['zone'] = hourly_df['timestamp'].apply(lambda ts: tariff_manager.get_zone_and_price(ts, tariff)[0])
        pobor_w_strefie_wysokiej = hourly_df[(hourly_df['date'].isin(net_import_days['date'])) & (hourly_df['zone'] == 'wysoka')].groupby('date')['pobor_przed'].sum()
